 ...
 void WITMotionNode::publisher()
{
    ...
    {
        if (!rtc_pub_)
            rtc_pub_ = this->create_publisher< builtin_interfaces::msg::Time>(topic_name_ + "/rtc", 1000);
    ...
    {
        if (!imu_pub_)
            imu_pub_ = this->create_publisher<sensor_msgs::msg::Imu>(topic_name_ + "/imu", 1000);
    ...
    if (imu_acceleration_flag_)
    {
        if (!imu_temperature_pub_)
            imu_temperature_pub_ = this->create_publisher<sensor_msgs::msg::Temperature>(topic_name_ + "/imu_temperature", 1000);
    ...
    if (magnetometer_flag_)
    {
        if (!magnetometer_pub_)
            magnetometer_pub_ = this->create_publisher<sensor_msgs::msg::MagneticField>(topic_name_ + "/magnetometer", 1000);
        magnetometer_pub_->publish(magnetometer_msg_);

        if (!magnetometer_temperature_pub_)
            magnetometer_temperature_pub_ = this->create_publisher<sensor_msgs::msg::Temperature>(topic_name_ + "/magnetometer_temperature", 1000);
    ...
    {
        if (!barometer_pub_)
            barometer_pub_ = this->create_publisher<sensor_msgs::msg::FluidPressure>(topic_name_ + "/barometer", 1000);
        barometer_pub_->publish(barometer_msg_);

        if (!altitude_pub_)
            altitude_pub_ = this->create_publisher<std_msgs::msg::Float64>(topic_name_ + "/altitude", 1000);
    ...
    {
        if (!gps_pub_)
            gps_pub_ = this->create_publisher<sensor_msgs::msg::NavSatFix>( topic_name_ + "/gps_location", 1000);
        gps_pub_->publish(gps_msg_);

        if (!gps_ground_speed_pub_)
            gps_ground_speed_pub_ = this->create_publisher<geometry_msgs::msg::Twist>( topic_name_ + "/gps_ground_speed", 1000);
        gps_ground_speed_pub_->publish(gps_ground_speed_msg_);

        if (!gps_satellites_pub_)
            gps_satellites_pub_ = this->create_publisher<std_msgs::msg::UInt32>( topic_name_ + "/gps_satellite_number", 1000);
        gps_satellites_pub_->publish(gps_satellites_msg_);

        if (!gps_variance_pub_)
            gps_variance_pub_ = this->create_publisher<geometry_msgs::msg::Vector3>( topic_name_ + "/gps_variance", 1000);
    ...
    {
        if (!orientation_pub_)
            orientation_pub_ = this->create_publisher<geometry_msgs::msg::Quaternion>( topic_name_ + "/orientation", 1000);
