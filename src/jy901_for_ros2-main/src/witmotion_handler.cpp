...
void WITMotionHandler::handleAcceleration(std::vector<double> &acceleration, double &acceleration_temperature, const std::vector<uint8_t> &data_buff)
{
    ...
    // 根据JY901协议：加速度量程±16g，转换公式：(raw_value / 32768) * 16 * 9.80665
    // 坐标系转换：传感器坐标系(X右,Y前,Z上) -> ROS坐标系(X前,Y左,Z上)
    // ROS_X = sensor_Y, ROS_Y = -sensor_X, ROS_Z = -sensor_Z (重力方向向下为负)
    acceleration[0] = static_cast<double>(temp[1]) / 32768.0 * 16 * 9.80665;  // ROS X = sensor Y
    acceleration[1] = -static_cast<double>(temp[0]) / 32768.0 * 16 * 9.80665; // ROS Y = -sensor X
    acceleration[2] = -static_cast<double>(temp[2]) / 32768.0 * 16 * 9.80665; // ROS Z = -sensor Z (重力向下)
